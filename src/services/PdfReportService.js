 ...
 * and rigorous single-page geometry.
 */
export class PdfReportService {
  ...
  static async generateReport({
    ...
    // ========================================================
    // 1. TOP HEADER BANNER (Y: 10 to 38)
    // ========================================================
    const headerY = 10;
    ...
    doc.text(`Team Benchmark: ${defaultTeamSize} Engineers`, pageWidth - margin - 7, headerY + 17, { align: 'right' });
    doc.setTextColor(52, 211, 153); // Emerald
    doc.text('Status: 100% Verified DORA Framework', pageWidth - margin - 7, headerY + 23, { align: 'right' });

    // ========================================================
    // 2. EXECUTIVE SCORECARD SUMMARY (Y: 42 to 72)
    // ========================================================
    const scoreY = 42;
    ...
    doc.roundedRect(margin + 3.5, scoreY + 3.5, 42, scoreHeight - 7, 3, 3, 'F');

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);
    doc.setTextColor(216, 180, 82);
    doc.text(`${totalScore}`, margin + 24.5, scoreY + 14, { align: 'center' });

    doc.setFontSize(8);
    doc.setTextColor(203, 213, 225);
    doc.text('HEALTH INDEX / 100', margin + 24.5, scoreY + 19, { align: 'center' });
    ...
    doc.text(tierName.toUpperCase(), margin + 24.5, scoreY + 23.5, { align: 'center' });

    // Right Narrative Block
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10.5);
    doc.setTextColor(15, 23, 42);
    doc.text(`Velocity Classification: ${tierName}`, margin + 50, scoreY + 9);
    ...
    doc.text(splitDesc, margin + 50, scoreY + 15.5);

    // ========================================================
    // 3. THREE QUANTIFIED IMPACT TILES (Y: 76 to 104)
    // ========================================================
    const metricY = 76;
    const metricHeight = 26;
    const colGap = 4;
    ...
    doc.text('ANNUAL CAPACITY RECLAIMED', margin + 5, metricY + 6.5);

    doc.setFontSize(13);
    doc.setTextColor(15, 23, 42);
    doc.text(`+${hoursSavedPerYear.toLocaleString()} hrs`, margin + 5, metricY + 15);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.setTextColor(16, 185, 129);
    doc.text(`Across ${defaultTeamSize} engineering seats`, margin + 5, metricY + 21);
    ...
    doc.setFontSize(7);
    doc.setTextColor(100, 116, 139);
    doc.text('PRODUCTIVITY DIVIDEND', col2X + 5, metricY + 6.5);

    doc.setFontSize(13);
    doc.setTextColor(180, 83, 9); // Amber gold
    doc.text(`$${annualSavingsDollars.toLocaleString()} USD`, col2X + 5, metricY + 15);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.setTextColor(16, 185, 129);
    doc.text('Capital efficiency reclaimed', col2X + 5, metricY + 21);
    ...
    doc.setFontSize(7);
    doc.setTextColor(100, 116, 139);
    doc.text('CYCLE TIME ACCELERATION', col3X + 5, metricY + 6.5);

    doc.setFontSize(13);
    doc.setTextColor(104, 51, 255); // Orchid violet
    doc.text('4.2x Faster PRs', col3X + 5, metricY + 15);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.setTextColor(100, 116, 139);
    doc.text('Target: -72% idle review wait', col3X + 5, metricY + 21);

    // ========================================================
    // 4. DORA MATURITY DIAGNOSTIC BREAKDOWN (Y: 108 to 178)
    // ========================================================
    const doraY = 108;
    ...
    doc.setFontSize(9.5);
    doc.setTextColor(15, 23, 42);
    doc.text('DORA MATURITY & DIAGNOSTIC DIMENSIONS', margin, doraY);
    ...
    questionsMeta.forEach((meta, idx) => {
      ...
      // Left Index Pill
      doc.setFillColor(11, 12, 51);
      doc.roundedRect(margin + 3, rowY + 3, 5, 5, 1, 1, 'F');
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(6.5);
      doc.setTextColor(216, 180, 82);
      doc.text(`${idx + 1}`, margin + 5.5, rowY + 6.8, { align: 'center' });

      // Dimension Name
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8.5);
      doc.setTextColor(15, 23, 42);
      doc.text(meta.id, margin + 11, rowY + 5.5);

      // Subtitle
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(6.5);
      doc.setTextColor(100, 116, 139);
      doc.text(meta.subtitle, margin + 11, rowY + 9);

      // Selected Value
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(7);
      doc.setTextColor(51, 65, 85);
      const truncated = text.length > 78 ? text.slice(0, 75) + '...' : text;
      doc.text(`Selected: "${truncated}"`, margin + 11, rowY + 12.8);
      ...
      doc.text(`${label.toUpperCase()} (${score}/25)`, pageWidth - margin - 4, rowY + 8.5, { align: 'right' });

      rowY += rowHeight + 2;
    });

    // ========================================================
    // 5. TAILORED NOVA ARCHITECTURAL PRESCRIPTIONS (Y: 180 to 248)
    // ========================================================
    const rxY = 180;
    ...
    doc.setFontSize(9.5);
    doc.setTextColor(15, 23, 42);
    doc.text('TAILORED NOVA ARCHITECTURAL PRESCRIPTIONS', margin, rxY);
    ...
    prescriptions.forEach((item, idx) => {
      ...
      // Gold vertical strip
      doc.setFillColor(idx === 0 ? 216 : idx === 1 ? 142 : 52, idx === 0 ? 180 : idx === 1 ? 111 : 211, idx === 0 ? 82 : idx === 1 ? 255 : 153);
      doc.rect(margin, pY + 2.5, 1.5, pHeight - 5, 'F');

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8.5);
      doc.setTextColor(15, 23, 42);
      doc.text(item.title, margin + 5, pY + 5.5);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7.2);
      doc.setTextColor(71, 85, 105);
      const pDesc = doc.splitTextToSize(item.desc, contentWidth - 40);
      doc.text(pDesc, margin + 5, pY + 10);

      // Pill on Right
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(7);
      doc.setTextColor(11, 12, 51);
      doc.text(item.badge, pageWidth - margin - 4, pY + 6.5, { align: 'right' });

      pY += pHeight + 2.5;
    });

    // ========================================================
    // 6. OFFICIAL SECURITY AUDIT & FOOTER SEAL (Y: 254 to 284)
    // ========================================================
    const footerY = 254;
    ...
    doc.setFontSize(7);
    doc.setTextColor(71, 85, 105);
    doc.text('SECURITY & COMPLIANCE:', margin, footerY + 5);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(216, 180, 82);
    doc.text(`AUDIT ID: NOV-ROI-${totalScore}-${Date.now().toString().slice(-6)}`, pageWidth - margin, footerY + 11, {
      align: 'right',
    });
