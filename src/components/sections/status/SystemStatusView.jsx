...
export function SystemStatusView({ onClose: _onClose }) {
  ...
  return (
    <div className="space-y-6">
      {/* Top Overview Ribbon */}
      <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-emerald-500/10 via-zinc-950/60 to-emerald-500/10 border border-emerald-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white dark:bg-zinc-900/80 border border-slate-200 dark:border-white/10 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-white transition-colors cursor-pointer active:scale-95"
          >
            <RotateCcw className={`w-3.5 h-3.5 ${isRefreshing ? 'animate-spin text-[#FF5500]' : ''}`} />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-white/10">
            ...
            <div
              ...
              className={`p-3.5 rounded-xl border transition-all ${
                reg.status.includes('Degraded')
                  ...
                  : 'bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-white/10 hover:border-[#FF5500]/40'
              }`}
            >
              ...
              <div className="flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-200 dark:border-white/5">
                ...
                <span
                  className={`font-bold ${
                    reg.basePing > 100 ? 'text-rose-400' : 'text-[#FF5500]'
                  }`}
                >
                  {reg.basePing}ms
                </span>
      ...
      <div className="p-4 rounded-xl bg-white/80 dark:bg-zinc-900/80 border border-slate-200 dark:border-white/10 space-y-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl text-xs font-extrabold bg-[#FF5500] hover:bg-[#FF6600] text-black transition-all cursor-pointer disabled:opacity-50 shrink-0 shadow-md shadow-orange-500/25 active:scale-95"
          >
        ...
        {chaosLogs.length > 0 && (
          <div className="p-3 rounded-lg bg-black/90 border border-white/10 font-mono text-[11px] space-y-1 text-slate-300 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Incident History */}
        <div className="p-4 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-white/10 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-[#FF5500]" /> Past Incidents (Last 30 Days)
            </span>
          ...
          <div className="space-y-2.5">
            {PAST_INCIDENTS.map((inc) => (
              <div
                key={inc.id}
                className="p-2.5 rounded-lg bg-white dark:bg-zinc-900/90 border border-slate-200 dark:border-white/5 space-y-1"
              >
                <div className="flex items-center justify-between text-[11px]">
                  <span className="font-mono text-[#FF5500] font-bold">{inc.id}</span>
        ...
        <div className="p-4 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-white/10 flex flex-col justify-between space-y-3">
          <div>
            <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
              <Bell className="w-3.5 h-3.5 text-[#FF5500]" />
            ...
            <form onSubmit={handleSubscribe} className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-xs rounded-xl bg-white dark:bg-zinc-900/90 border border-slate-200 dark:border-white/15 text-slate-900 dark:text-white focus:outline-none focus:border-[#FF5500] focus:ring-1 focus:ring-[#FF5500]"
                />
                <button
                  type="submit"
                  className="px-3.5 py-2 bg-[#FF5500] hover:bg-[#FF6600] text-black text-xs font-extrabold rounded-xl flex items-center gap-1 cursor-pointer transition-colors shrink-0 shadow-md shadow-orange-500/25 active:scale-95"
                >
